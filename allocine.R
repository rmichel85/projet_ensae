...
getwd()
setwd("\\\\telemaque/users/CEPE-S1-09/AAA")
...
nom_save<-paste0(id,"_01principal",".csv")
write.csv(url_principal_brut,nom_save)
...
GET(affiche, write_disk("forme_eau.jpg"))
# im <- load.image("hostiles.jpg")
# plot(im)

date_sortie<- principal %>% html_nodes(".date") %>% html_text()
duree<- principal %>% html_nodes(".meta-body-item:nth-child(1)") %>% html_text()
duree<- gsub("\\n","",duree)
duree<- gsub(" ","",duree)
duree<- substr(duree,nchar(duree)-7,nchar(duree)-1)

genre1<- principal %>% html_nodes(".meta-body-item .light+ .blue-link span") %>% html_text()
genresSup<- principal %>% html_nodes(".blue-link+ .blue-link span") %>% html_text()
genres<-c(genre1,genresSup)
...
nom_save<-paste0(id,"_02casting",".txt")
...
# for (cpt in seq_along(names_autres_tout) ) {
#   autres_fonctions<-gsub(names_autres_tout[cpt],"",setdiff(autres_fonctions,"Production"))
# }
# for (cpt2 in seq_along(autres) ) {
#   autres_fonctions<-gsub(autres[cpt2],"",autres_fonctions)
# }
